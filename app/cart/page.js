    ...
    const deleteCartItemFromList = (itemId) => {
        CartApis.deleteCartItem(itemId)
            .then((res) => {
                if (res) {
                    setCart((oldCart) => {
                        return oldCart.filter(item => item.id !== itemId); // Return the new filtered cart array
                    });
                }
            })
            .catch((err) => {
                            ...
                            {cart?.map((item) => (
                                <li key={item?.id} className="flex items-center gap-4">
                                    ...
                                    <div className="flex flex-1 items-center justify-end gap-2">
                                        ...
                                        <button onClick={() => deleteCartItemFromList(item?.id)} className="text-gray-600 ml-3 transition hover:text-red-600">
